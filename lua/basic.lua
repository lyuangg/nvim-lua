...
-- vim.opt.grepprg =
-- "grep -rn $* --exclude-dir=.git"                                                                                         -- grep 设置
...
vim.keymap.set("v", "<leader>js", function()
    return "!jsonformat.php " ..
        vim.fn.input("type name: ", "T1") .. " " .. vim.fn.input("tag name: ", "json") .. " " .. "<cr>"
end, { expr = true })


vim.cmd('command! Diffu :w !diff % -')
vim.cmd('packadd cfilter')
vim.keymap.set("n", "<leader>ll", ":echo<CR>", { silent = true, noremap = true })
vim.keymap.set("i", "<c-j>", "<End><CR>", { silent = true, noremap = true })
vim.cmd([[
  augroup php_mappings
    autocmd!
    autocmd FileType php inoremap <buffer> <silent> <c-j> <End>;<CR>
  augroup END
]])
